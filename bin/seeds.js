...
const MONGO_URI = process.env.MONGODB_URI || 'mongodb://localhost/library-project'; // declare a variable if there is an env with MONGO-Uri we will use it,otherwise mongodb...

mongoose.connect(MONGO_URI, { //try to connect database wit some settings
...
Book.create(books) //on the model,passing a array of obj create on DB
  .then(booksFromDB => {
